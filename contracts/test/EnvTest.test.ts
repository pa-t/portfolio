...
describe('Is Env file correctly configured', function () {
  ...
  describe('mainnet', async function () {
    it('has ETH_NODE_URI_MAINNET', async function () {
      expect(ETH_NODE_URI_MAINNET, 'did you add ETH_NODE_URI_MAINNET to .env?')
        .to.not.be.undefined;
    });
    ...
    it('has usable mainnet provider', async function () {
      expect(
        ETH_NODE_URI_MAINNET,
        'is ETH_NODE_URI_MAINNET the full alchemy http url?'
      ).to.include('https://eth-mainnet.alchemyapi.io/v2/');
      expect(
        ETH_NODE_URI_MAINNET?.length,
        'did you add the API key at the end of the URL?'
      ).to.equal(69);
    });
  });

  describe('rinkeby', async function () {
    it('has ETH_NODE_URI_RINKEBY', async function () {
      expect(ETH_NODE_URI_RINKEBY, 'did you add ETH_NODE_URI_RINKEBY to .env?')
        .to.not.be.undefined;
    });

    it('has usable rinkeby provider', async function () {
      expect(
        ETH_NODE_URI_RINKEBY,
        'is ETH_NODE_URI_RINKEBY the full alchemy http url?'
      ).to.include('https://eth-rinkeby.alchemyapi.io/v2/');
      expect(
        ETH_NODE_URI_RINKEBY?.length,
        'did you add the API key at the end of the URL?'
      ).to.equal(69);
    });
  });

  describe('ropsten', async function () {
    it('has ETH_NODE_URI_ROPSTEN', async function () {
      expect(ETH_NODE_URI_ROPSTEN, 'did you add ETH_NODE_URI_ROPSTEN to .env?')
        .to.not.be.undefined;
    });

    it('has usable ropsten provider', async function () {
      expect(
        ETH_NODE_URI_ROPSTEN,
        'is ETH_NODE_URI_ROPSTEN the full alchemy http url?'
      ).to.include('https://eth-ropsten.alchemyapi.io/v2/');
      expect(
        ETH_NODE_URI_ROPSTEN?.length,
        'did you add the API key at the end of the URL?'
      ).to.equal(69);
    });
  });

  describe('kovan', async function () {
    it('has ETH_NODE_URI_KOVAN', async function () {
      expect(ETH_NODE_URI_KOVAN, 'did you add ETH_NODE_URI_KOVAN to .env?').to
        .not.be.undefined;
    });
    ...
    it('has usable kovan provider', async function () {
      expect(
        ETH_NODE_URI_KOVAN,
        'is ETH_NODE_URI_KOVAN the full alchemy http url?'
      ).to.include('https://eth-kovan.alchemyapi.io/v2/');
      expect(
        ETH_NODE_URI_KOVAN?.length,
        'did you add the API key at the end of the URL?'
      ).to.equal(67);
    });
  });
  ...
  describe('mnemonic', async function () {
    ...
    it('mnemonic is 12 words', async function () {
      ...
      expect(
        mnemonicWordCount,
        'incorrect length for mnemonic, run npx mnemonics'
      ).to.equal(12);
    });
  });
